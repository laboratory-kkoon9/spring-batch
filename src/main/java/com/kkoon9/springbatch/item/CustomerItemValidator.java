...
public class CustomerItemValidator implements Validator<CustomerUpdate> {
    ...
    public void validate(CustomerUpdate customer) throws ValidationException {
        Map<String, Long> parameters = Collections.singletonMap("id", customer.getCustomerId());

        Long count = jdbcTemplate.queryForObject(FIND_CUSTOMER, parameters, Long.class);
